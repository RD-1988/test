...
# metric for GDP per capita
gdpPerCapita <- wbApi(indicator = "NY.GDP.PCAP.CD", country = "all", start = 2018, end = 2018) #GDP per capita (current US$)
colnames(gdpPerCapita)[3:length(colnames(gdpPerCapita))] <- paste("gdp", colnames(gdpPerCapita)[3:length(colnames(gdpPerCapita))], sep = "")
gdpPerCapita$iso3c <- countrycode(gdpPerCapita$country, origin = 'country.name', destination = 'genc3c')
gdpPerCapita

# another metric that you consider could be correlated to language fractionalization
gnpPerCapita <- wbApi(indicator = "NY.GNP.PCAP.CD", country =  "all", start = 2018, end = 2018) #GNI per capita, Atlas method (current US$)
colnames(gnpPerCapita)[3:length(colnames(gnpPerCapita))] <- paste("gnp", colnames(gnpPerCapita)[3:length(colnames(gnpPerCapita))], sep = "")
gnpPerCapita$iso3c <- countrycode(gnpPerCapita$country, origin = 'country.name', destination = 'genc3c')
gnpPerCapita
...
```{r}
my_metric <- read_csv("my_metric.csv")
names(my_metric)[2] <- "iso3c.x"
mergedData <- merge(my_metric, countryLevelData, by= "iso3c.x") # the Bailiwick of Jersey & Guernsey were not merged as these two crown dependencies are not accounted as countries by the world bank
...
```
I replicate the findings from the paper. There is a slight correlation between language fractionalization and economics development from this new dataset.